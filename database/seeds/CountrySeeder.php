<?php

use Illuminate\Database\Seeder;

class CountrySeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {

        App\Countries::create( [
            'id'=>1,
            'name'=>'China',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>2,
            'name'=>'India',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>3,
            'name'=>'United States',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>4,
            'name'=>'Indonesia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>5,
            'name'=>'Brazil',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>6,
            'name'=>'Pakistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>7,
            'name'=>'Nigeria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>8,
            'name'=>'Bangladesh',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>9,
            'name'=>'Russia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>10,
            'name'=>'Japan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>11,
            'name'=>'Mexico',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>12,
            'name'=>'Philippines',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>13,
            'name'=>'Vietnam',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>14,
            'name'=>'Ethiopia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>15,
            'name'=>'Egypt',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>16,
            'name'=>'Germany',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>17,
            'name'=>'Iran',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>18,
            'name'=>'Turkey',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>19,
            'name'=>'Democratic Republic of the Congo',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>20,
            'name'=>'Thailand',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>21,
            'name'=>'France',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>22,
            'name'=>'United Kingdom',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>23,
            'name'=>'Italy',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>24,
            'name'=>'Burma',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>25,
            'name'=>'South Africa',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>26,
            'name'=>'South Korea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>27,
            'name'=>'Colombia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>28,
            'name'=>'Spain',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>29,
            'name'=>'Ukraine',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>30,
            'name'=>'Tanzania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>31,
            'name'=>'Kenya',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>32,
            'name'=>'Argentina',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>33,
            'name'=>'Algeria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>34,
            'name'=>'Poland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>35,
            'name'=>'Sudan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>36,
            'name'=>'Uganda',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>37,
            'name'=>'Canada',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>38,
            'name'=>'Iraq',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>39,
            'name'=>'Morocco',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>40,
            'name'=>'Peru',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>41,
            'name'=>'Uzbekistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>42,
            'name'=>'Saudi Arabia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>43,
            'name'=>'Malaysia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>44,
            'name'=>'Venezuela',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>45,
            'name'=>'Nepal',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>46,
            'name'=>'Afghanistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>47,
            'name'=>'Yemen',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>48,
            'name'=>'North Korea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>49,
            'name'=>'Ghana',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>50,
            'name'=>'Mozambique',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>51,
            'name'=>'Taiwan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>52,
            'name'=>'Australia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>53,
            'name'=>'Ivory Coast',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>54,
            'name'=>'Syria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>55,
            'name'=>'Madagascar',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>56,
            'name'=>'Angola',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>57,
            'name'=>'Cameroon',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>58,
            'name'=>'Sri Lanka',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>59,
            'name'=>'Romania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>60,
            'name'=>'Burkina Faso',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>61,
            'name'=>'Niger',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>62,
            'name'=>'Kazakhstan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>63,
            'name'=>'Netherlands',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>64,
            'name'=>'Chile',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>65,
            'name'=>'Malawi',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>66,
            'name'=>'Ecuador',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>67,
            'name'=>'Guatemala',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>68,
            'name'=>'Mali',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>69,
            'name'=>'Cambodia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>70,
            'name'=>'Senegal',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>71,
            'name'=>'Zambia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>72,
            'name'=>'Zimbabwe',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>73,
            'name'=>'Chad',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>74,
            'name'=>'South Sudan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>75,
            'name'=>'Belgium',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>76,
            'name'=>'Cuba',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>77,
            'name'=>'Tunisia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>78,
            'name'=>'Guinea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>79,
            'name'=>'Greece',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>80,
            'name'=>'Portugal',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>81,
            'name'=>'Rwanda',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>82,
            'name'=>'Czech Republic',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>83,
            'name'=>'Somalia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>84,
            'name'=>'Haiti',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>85,
            'name'=>'Benin',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>86,
            'name'=>'Burundi',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>87,
            'name'=>'Bolivia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>88,
            'name'=>'Hungary',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>89,
            'name'=>'Sweden',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>90,
            'name'=>'Belarus',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>91,
            'name'=>'Dominican Republic',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>92,
            'name'=>'Azerbaijan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>93,
            'name'=>'Honduras',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>94,
            'name'=>'Austria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>95,
            'name'=>'United Arab Emirates',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>96,
            'name'=>'Israel',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>97,
            'name'=>'Switzerland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>98,
            'name'=>'Tajikistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>99,
            'name'=>'Bulgaria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>100,
            'name'=>'Hong Kong (China)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>101,
            'name'=>'Serbia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>102,
            'name'=>'Papua New Guinea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>103,
            'name'=>'Paraguay',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>104,
            'name'=>'Laos',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>105,
            'name'=>'Jordan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>106,
            'name'=>'El Salvador',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>107,
            'name'=>'Eritrea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>108,
            'name'=>'Libya',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>109,
            'name'=>'Togo',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>110,
            'name'=>'Sierra Leone',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>111,
            'name'=>'Nicaragua',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>112,
            'name'=>'Kyrgyzstan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>113,
            'name'=>'Denmark',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>114,
            'name'=>'Finland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>115,
            'name'=>'Slovakia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>116,
            'name'=>'Singapore',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>117,
            'name'=>'Turkmenistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>118,
            'name'=>'Norway',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>119,
            'name'=>'Lebanon',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>120,
            'name'=>'Costa Rica',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>121,
            'name'=>'Central African Republic',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>122,
            'name'=>'Ireland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>123,
            'name'=>'Georgia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>124,
            'name'=>'New Zealand',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>125,
            'name'=>'Republic of the Congo',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>126,
            'name'=>'Palestine',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>127,
            'name'=>'Liberia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>128,
            'name'=>'Croatia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>129,
            'name'=>'Oman',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>130,
            'name'=>'Bosnia and Herzegovina',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>131,
            'name'=>'Puerto Rico',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>132,
            'name'=>'Kuwait',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>133,
            'name'=>'Moldov',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>134,
            'name'=>'Mauritania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>135,
            'name'=>'Panama',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>136,
            'name'=>'Uruguay',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>137,
            'name'=>'Armenia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>138,
            'name'=>'Lithuania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>139,
            'name'=>'Albania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>140,
            'name'=>'Mongolia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>141,
            'name'=>'Jamaica',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>142,
            'name'=>'Namibia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>143,
            'name'=>'Lesotho',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>144,
            'name'=>'Qatar',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>145,
            'name'=>'Macedonia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>146,
            'name'=>'Slovenia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>147,
            'name'=>'Botswana',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>148,
            'name'=>'Latvia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>149,
            'name'=>'Gambia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>150,
            'name'=>'Kosovo',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>151,
            'name'=>'Guinea-Bissau',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>152,
            'name'=>'Gabon',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>153,
            'name'=>'Equatorial Guinea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>154,
            'name'=>'Trinidad and Tobago',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>155,
            'name'=>'Estonia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>156,
            'name'=>'Mauritius',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>157,
            'name'=>'Swaziland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>158,
            'name'=>'Bahrain',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>159,
            'name'=>'Timor-Leste',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>160,
            'name'=>'Djibouti',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>161,
            'name'=>'Cyprus',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>162,
            'name'=>'Fiji',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>163,
            'name'=>'Reunion (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>164,
            'name'=>'Guyana',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>165,
            'name'=>'Comoros',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>166,
            'name'=>'Bhutan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>167,
            'name'=>'Montenegro',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>168,
            'name'=>'Macau (China)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>169,
            'name'=>'Solomon Islands',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>170,
            'name'=>'Western Sahara',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>171,
            'name'=>'Luxembourg',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>172,
            'name'=>'Suriname',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>173,
            'name'=>'Cape Verde',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>174,
            'name'=>'Malta',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>175,
            'name'=>'Guadeloupe (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>176,
            'name'=>'Martinique (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>177,
            'name'=>'Brunei',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>178,
            'name'=>'Bahamas',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>179,
            'name'=>'Iceland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>180,
            'name'=>'Maldives',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>181,
            'name'=>'Belize',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>182,
            'name'=>'Barbados',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>183,
            'name'=>'French Polynesia (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>184,
            'name'=>'Vanuatu',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>185,
            'name'=>'New Caledonia (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>186,
            'name'=>'French Guiana (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>187,
            'name'=>'Mayotte (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>188,
            'name'=>'Samoa',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>189,
            'name'=>'Sao Tom and Principe',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>190,
            'name'=>'Saint Lucia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>191,
            'name'=>'Guam (USA)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>192,
            'name'=>'Curacao (Netherlands)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>193,
            'name'=>'Saint Vincent and the Grenadines',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>194,
            'name'=>'Kiribati',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>195,
            'name'=>'United States Virgin Islands (USA)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>196,
            'name'=>'Grenada',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>197,
            'name'=>'Tonga',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>198,
            'name'=>'Aruba (Netherlands)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>199,
            'name'=>'Federated States of Micronesia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>200,
            'name'=>'Jersey (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>201,
            'name'=>'Seychelles',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>202,
            'name'=>'Antigua and Barbuda',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>203,
            'name'=>'Isle of Man (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>204,
            'name'=>'Andorra',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>205,
            'name'=>'Dominica',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>206,
            'name'=>'Bermuda (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>207,
            'name'=>'Guernsey (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>208,
            'name'=>'Greenland (Denmark)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>209,
            'name'=>'Marshall Islands',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>210,
            'name'=>'American Samoa (USA)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>211,
            'name'=>'Cayman Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>212,
            'name'=>'Saint Kitts and Nevis',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>213,
            'name'=>'Northern Mariana Islands (USA)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>214,
            'name'=>'Faroe Islands (Denmark)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>215,
            'name'=>'Sint Maarten (Netherlands)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>216,
            'name'=>'Saint Martin (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>217,
            'name'=>'Liechtenstein',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>218,
            'name'=>'Monaco',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>219,
            'name'=>'San Marino',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>220,
            'name'=>'Turks and Caicos Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>221,
            'name'=>'Gibraltar (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>222,
            'name'=>'British Virgin Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>223,
            'name'=>'Aland Islands (Finland)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>224,
            'name'=>'Caribbean Netherlands (Netherlands)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>225,
            'name'=>'Palau',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>226,
            'name'=>'Cook Islands (NZ)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>227,
            'name'=>'Anguilla (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>228,
            'name'=>'Wallis and Futuna (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>229,
            'name'=>'Tuvalu',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>230,
            'name'=>'Nauru',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>231,
            'name'=>'Saint Barthelemy (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>232,
            'name'=>'Saint Pierre and Miquelon (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>233,
            'name'=>'Montserrat (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>234,
            'name'=>'"Saint Helena',
            'created_at'=>NULL,
            'updated_at'=>NULL,
            =>)
            ] );

            App\Countries::create( [
            'id'=>235,
            'name'=>'Svalbard and Jan Mayen (Norway)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>236,
            'name'=>'Falkland Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>237,
            'name'=>'Norfolk Island (Australia)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>238,
            'name'=>'Christmas Island (Australia)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>239,
            'name'=>'Niue (NZ)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>240,
            'name'=>'Tokelau (NZ)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>241,
            'name'=>'Vatican City',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>242,
            'name'=>'Cocos (Keeling) Islands (Australia)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>243,
            'name'=>'Pitcairn Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>/*!40000 ALTER TABLE `countries` ENABLE KEYS */
            ] );

            App\Countries::create( [
            'id'=>/*!40101 SET SQL_MODE=IFNULL(@OLD_SQL_MODE, '') */
            ] );

            App\Countries::create( [
            'id'=>/*!40014 SET FOREIGN_KEY_CHECKS=IF(@OLD_FOREIGN_KEY_CHECKS IS NULL, 1, @OLD_FOREIGN_KEY_CHECKS) */
            ] );

            App\Countries::create( [
            'id'=>/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */
            ] );
                        ","1","","112.196.72.114","Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.90 Safari/537.36","https://www.google.com/")
            App\Countries::create( [
            'id'=>1,
            'name'=>'China',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>2,
            'name'=>'India',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>3,
            'name'=>'United States',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>4,
            'name'=>'Indonesia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>5,
            'name'=>'Brazil',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>6,
            'name'=>'Pakistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>7,
            'name'=>'Nigeria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>8,
            'name'=>'Bangladesh',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>9,
            'name'=>'Russia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>10,
            'name'=>'Japan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>11,
            'name'=>'Mexico',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>12,
            'name'=>'Philippines',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>13,
            'name'=>'Vietnam',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>14,
            'name'=>'Ethiopia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>15,
            'name'=>'Egypt',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>16,
            'name'=>'Germany',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>17,
            'name'=>'Iran',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>18,
            'name'=>'Turkey',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>19,
            'name'=>'Democratic Republic of the Congo',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>20,
            'name'=>'Thailand',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>21,
            'name'=>'France',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>22,
            'name'=>'United Kingdom',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>23,
            'name'=>'Italy',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>24,
            'name'=>'Burma',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>25,
            'name'=>'South Africa',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>26,
            'name'=>'South Korea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>27,
            'name'=>'Colombia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>28,
            'name'=>'Spain',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>29,
            'name'=>'Ukraine',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>30,
            'name'=>'Tanzania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>31,
            'name'=>'Kenya',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>32,
            'name'=>'Argentina',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>33,
            'name'=>'Algeria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>34,
            'name'=>'Poland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>35,
            'name'=>'Sudan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>36,
            'name'=>'Uganda',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>37,
            'name'=>'Canada',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>38,
            'name'=>'Iraq',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>39,
            'name'=>'Morocco',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>40,
            'name'=>'Peru',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>41,
            'name'=>'Uzbekistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>42,
            'name'=>'Saudi Arabia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>43,
            'name'=>'Malaysia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>44,
            'name'=>'Venezuela',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>45,
            'name'=>'Nepal',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>46,
            'name'=>'Afghanistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>47,
            'name'=>'Yemen',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>48,
            'name'=>'North Korea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>49,
            'name'=>'Ghana',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>50,
            'name'=>'Mozambique',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>51,
            'name'=>'Taiwan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>52,
            'name'=>'Australia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>53,
            'name'=>'Ivory Coast',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>54,
            'name'=>'Syria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>55,
            'name'=>'Madagascar',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>56,
            'name'=>'Angola',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>57,
            'name'=>'Cameroon',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>58,
            'name'=>'Sri Lanka',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>59,
            'name'=>'Romania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>60,
            'name'=>'Burkina Faso',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>61,
            'name'=>'Niger',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>62,
            'name'=>'Kazakhstan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>63,
            'name'=>'Netherlands',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>64,
            'name'=>'Chile',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>65,
            'name'=>'Malawi',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>66,
            'name'=>'Ecuador',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>67,
            'name'=>'Guatemala',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>68,
            'name'=>'Mali',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>69,
            'name'=>'Cambodia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>70,
            'name'=>'Senegal',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>71,
            'name'=>'Zambia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>72,
            'name'=>'Zimbabwe',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>73,
            'name'=>'Chad',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>74,
            'name'=>'South Sudan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>75,
            'name'=>'Belgium',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>76,
            'name'=>'Cuba',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>77,
            'name'=>'Tunisia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>78,
            'name'=>'Guinea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>79,
            'name'=>'Greece',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>80,
            'name'=>'Portugal',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>81,
            'name'=>'Rwanda',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>82,
            'name'=>'Czech Republic',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>83,
            'name'=>'Somalia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>84,
            'name'=>'Haiti',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>85,
            'name'=>'Benin',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>86,
            'name'=>'Burundi',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>87,
            'name'=>'Bolivia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>88,
            'name'=>'Hungary',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>89,
            'name'=>'Sweden',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>90,
            'name'=>'Belarus',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>91,
            'name'=>'Dominican Republic',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>92,
            'name'=>'Azerbaijan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>93,
            'name'=>'Honduras',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>94,
            'name'=>'Austria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>95,
            'name'=>'United Arab Emirates',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>96,
            'name'=>'Israel',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>97,
            'name'=>'Switzerland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>98,
            'name'=>'Tajikistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>99,
            'name'=>'Bulgaria',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>100,
            'name'=>'Hong Kong (China)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>101,
            'name'=>'Serbia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>102,
            'name'=>'Papua New Guinea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>103,
            'name'=>'Paraguay',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>104,
            'name'=>'Laos',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>105,
            'name'=>'Jordan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>106,
            'name'=>'El Salvador',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>107,
            'name'=>'Eritrea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>108,
            'name'=>'Libya',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>109,
            'name'=>'Togo',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>110,
            'name'=>'Sierra Leone',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>111,
            'name'=>'Nicaragua',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>112,
            'name'=>'Kyrgyzstan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>113,
            'name'=>'Denmark',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>114,
            'name'=>'Finland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>115,
            'name'=>'Slovakia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>116,
            'name'=>'Singapore',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>117,
            'name'=>'Turkmenistan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>118,
            'name'=>'Norway',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>119,
            'name'=>'Lebanon',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>120,
            'name'=>'Costa Rica',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>121,
            'name'=>'Central African Republic',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>122,
            'name'=>'Ireland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>123,
            'name'=>'Georgia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>124,
            'name'=>'New Zealand',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>125,
            'name'=>'Republic of the Congo',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>126,
            'name'=>'Palestine',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>127,
            'name'=>'Liberia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>128,
            'name'=>'Croatia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>129,
            'name'=>'Oman',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>130,
            'name'=>'Bosnia and Herzegovina',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>131,
            'name'=>'Puerto Rico',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>132,
            'name'=>'Kuwait',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>133,
            'name'=>'Moldov',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>134,
            'name'=>'Mauritania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>135,
            'name'=>'Panama',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>136,
            'name'=>'Uruguay',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>137,
            'name'=>'Armenia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>138,
            'name'=>'Lithuania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>139,
            'name'=>'Albania',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>140,
            'name'=>'Mongolia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>141,
            'name'=>'Jamaica',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>142,
            'name'=>'Namibia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>143,
            'name'=>'Lesotho',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>144,
            'name'=>'Qatar',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>145,
            'name'=>'Macedonia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>146,
            'name'=>'Slovenia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>147,
            'name'=>'Botswana',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>148,
            'name'=>'Latvia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>149,
            'name'=>'Gambia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>150,
            'name'=>'Kosovo',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>151,
            'name'=>'Guinea-Bissau',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>152,
            'name'=>'Gabon',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>153,
            'name'=>'Equatorial Guinea',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>154,
            'name'=>'Trinidad and Tobago',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>155,
            'name'=>'Estonia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>156,
            'name'=>'Mauritius',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>157,
            'name'=>'Swaziland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>158,
            'name'=>'Bahrain',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>159,
            'name'=>'Timor-Leste',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>160,
            'name'=>'Djibouti',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>161,
            'name'=>'Cyprus',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>162,
            'name'=>'Fiji',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>163,
            'name'=>'Reunion (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>164,
            'name'=>'Guyana',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>165,
            'name'=>'Comoros',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>166,
            'name'=>'Bhutan',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>167,
            'name'=>'Montenegro',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>168,
            'name'=>'Macau (China)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>169,
            'name'=>'Solomon Islands',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>170,
            'name'=>'Western Sahara',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>171,
            'name'=>'Luxembourg',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>172,
            'name'=>'Suriname',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>173,
            'name'=>'Cape Verde',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>174,
            'name'=>'Malta',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>175,
            'name'=>'Guadeloupe (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>176,
            'name'=>'Martinique (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>177,
            'name'=>'Brunei',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>178,
            'name'=>'Bahamas',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>179,
            'name'=>'Iceland',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>180,
            'name'=>'Maldives',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>181,
            'name'=>'Belize',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>182,
            'name'=>'Barbados',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>183,
            'name'=>'French Polynesia (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>184,
            'name'=>'Vanuatu',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>185,
            'name'=>'New Caledonia (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>186,
            'name'=>'French Guiana (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>187,
            'name'=>'Mayotte (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>188,
            'name'=>'Samoa',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>189,
            'name'=>'Sao Tom and Principe',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>190,
            'name'=>'Saint Lucia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>191,
            'name'=>'Guam (USA)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>192,
            'name'=>'Curacao (Netherlands)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>193,
            'name'=>'Saint Vincent and the Grenadines',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>194,
            'name'=>'Kiribati',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>195,
            'name'=>'United States Virgin Islands (USA)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>196,
            'name'=>'Grenada',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>197,
            'name'=>'Tonga',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>198,
            'name'=>'Aruba (Netherlands)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>199,
            'name'=>'Federated States of Micronesia',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>200,
            'name'=>'Jersey (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>201,
            'name'=>'Seychelles',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>202,
            'name'=>'Antigua and Barbuda',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>203,
            'name'=>'Isle of Man (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>204,
            'name'=>'Andorra',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>205,
            'name'=>'Dominica',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>206,
            'name'=>'Bermuda (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>207,
            'name'=>'Guernsey (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>208,
            'name'=>'Greenland (Denmark)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>209,
            'name'=>'Marshall Islands',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>210,
            'name'=>'American Samoa (USA)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>211,
            'name'=>'Cayman Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>212,
            'name'=>'Saint Kitts and Nevis',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>213,
            'name'=>'Northern Mariana Islands (USA)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>214,
            'name'=>'Faroe Islands (Denmark)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>215,
            'name'=>'Sint Maarten (Netherlands)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>216,
            'name'=>'Saint Martin (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>217,
            'name'=>'Liechtenstein',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>218,
            'name'=>'Monaco',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>219,
            'name'=>'San Marino',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>220,
            'name'=>'Turks and Caicos Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>221,
            'name'=>'Gibraltar (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>222,
            'name'=>'British Virgin Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>223,
            'name'=>'Aland Islands (Finland)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>224,
            'name'=>'Caribbean Netherlands (Netherlands)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>225,
            'name'=>'Palau',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>226,
            'name'=>'Cook Islands (NZ)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>227,
            'name'=>'Anguilla (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>228,
            'name'=>'Wallis and Futuna (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>229,
            'name'=>'Tuvalu',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>230,
            'name'=>'Nauru',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>231,
            'name'=>'Saint Barthelemy (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>232,
            'name'=>'Saint Pierre and Miquelon (France)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>233,
            'name'=>'Montserrat (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>234,
            'name'=>'"Saint Helena',
            'created_at'=>NULL,
            'updated_at'=>NULL,

            ] );

            App\Countries::create( [
            'id'=>235,
            'name'=>'Svalbard and Jan Mayen (Norway)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>236,
            'name'=>'Falkland Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>237,
            'name'=>'Norfolk Island (Australia)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>238,
            'name'=>'Christmas Island (Australia)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>239,
            'name'=>'Niue (NZ)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>240,
            'name'=>'Tokelau (NZ)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>241,
            'name'=>'Vatican City',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>242,
            'name'=>'Cocos (Keeling) Islands (Australia)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );

            App\Countries::create( [
            'id'=>243,
            'name'=>'Pitcairn Islands (UK)',
            'created_at'=>NULL,
            'updated_at'=>NULL
            ] );
    }
}
